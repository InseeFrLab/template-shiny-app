...
RUN Rscript -e "install.packages(c('aws.s3', 'readr', 'DBI', 'RPostgres', 'glue', 'shiny', 'leaflet'), repos='https://nexus.insee.fr/repository/r-cran/', method = 'wget', extra = '--no-check-certificate')"
...
# Endpoint
CMD ["Rscript", "-e", "myshinyapp::run_app()"]
